"""
Linear-probe AUROC for Merlin conditions — early-stopping variant.

Same purpose as run_probe_merlin.py (per-condition AUROC from cached
features), but a different regularization strategy. run_probe_merlin.py
trains one shared multi-output head for fixed epochs and grid-searches
weight decay; this script gives every condition its own independent
single-output linear probe (nn.Linear(d, 1)) and early-stops each one on
*that condition's own* validation AUROC. The reported "joint mean AUROC"
is the mean of these independently early-stopped per-condition AUROCs.

Cost: this is still just linear probes on top of frozen, pre-cached
features (no backbone forward pass) — d=2048, ~3.5k train rows, <2.1k
params per probe. Looping over ~10-30 conditions x n_seeds is more probe
instances than the joint version, but each one trains in a fraction of a
second; the whole sweep is GPU-seconds, not minutes.

Caveat carried over from run_probe_merlin.py: several conditions have very
few labeled val examples (see docs/merlin_label_stats.md), so per-epoch val
AUROC can be noisy for those, and early stopping inherits that noise — same
risk the original script avoided by dropping early stopping entirely, just
scoped to one condition's own probe instead of a shared loss term.
min_test_per_class still gates which conditions get reported.

Output results.json schema matches run_probe_merlin.py (mean_auroc,
std_auroc, per_condition, per_condition_std, seed_aucs), so the existing
plot_probe_merlin.py / plot_merlin_auroc.py can plot it directly.

Usage:
    python scripts/run_probe_merlin_alternative.py \\
        --feat_dir runs/qit_merlin_fulldata/features \\
        --output_dir runs/qit_merlin_fulldata/probe_results_alt

    python scripts/run_probe_merlin_alternative.py \\
        --feat_dir runs/teacher_features \\
        --output_dir runs/teacher_features/probe_results_alt
"""
...
def run_auroc_probe(train_feats, train_labels, val_feats, val_labels,
    ...
    # mirrors run_probe_merlin.py's seed_aucs semantics (one joint number per seed).
...
def main():
    ...
    parser.add_argument("--output_dir", default=None,
                        help="Where to write results.json. Defaults to {feat_dir}/probe_results_alt.")
    ...
    out = Path(args.output_dir) if args.output_dir else feat_dir / "probe_results_alt"
